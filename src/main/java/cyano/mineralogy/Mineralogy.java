...
@Mod(modid = Mineralogy.MODID, name = Mineralogy.NAME,dependencies = "required-after:multithreadingandtweaks@[0.8,);", version = Mineralogy.VERSION)
public class Mineralogy {
    ...
    public void preInit(FMLPreInitializationEvent event) {
       ...
       // sedimentaryStones.add(Blocks.gravel);
        ...
        String[] colorSuffixes = { "black", "red", "green", "brown", "blue", "purple", "cyan", "silver", "gray", "pink",
        ...
        GameRegistry.addRecipe(
            new ShapelessOreRecipe(
                ...
                NitrateDust.dictionaryName,
                SulfurDust.dictionaryName));
        GameRegistry.addRecipe(
            new ShapelessOreRecipe(
                new ItemStack(Items.gunpowder, 4),
                Items.sugar,
        ...
        GameRegistry.addRecipe(
            new ShapelessOreRecipe(
                new ItemStack(mineralFertilizer, 1),
                NitrateDust.dictionaryName,
                PhosphoriteDust.dictionaryName));
    ...
    private static void addOre(String oreName, String oreDictionaryName, Item oreDropItem, int numMin, int numMax,
        int pickLevel, int minY, int maxY, int spawnFrequency, int spawnQuantity) {
        String oreBlockName = Mineralogy.MODID + "_" + oreName;
        Block oreBlock = new Ore(oreName, oreDropItem, numMin, numMax, pickLevel);
        GameRegistry.registerBlock(oreBlock, oreName); // MUST REGISTER BLOCK WITH GAME BEFORE DOING ANYTHING ELSE WITH
                                                       // IT!!!
     ...
     * @param hardness                How hard (time duration) the block is to pick. For reference, dirt is 0.5, stone
     *                                is 1.5, ores are 3, and obsidian is 50
     * @param blastResistance         how resistant the block is to explosions. For reference, dirt is 0, stone is 10,
     *                                and blast-proof materials are 2000
    ...
    private static void addStoneType(RockType type, String name, double hardness, double blastResistance,
        int toolHardnessLevel, boolean isStoneEquivalent, boolean isCobblestoneEquivalent, boolean hasSmooth,
        boolean hasBricks) {
        Block b = new Rock();
        ...
        if (hasSmooth) {
            ...
            GameRegistry.registerBlock(b2, smoothName); // MUST REGISTER BLOCK WITH GAME BEFORE DOING ANYTHING ELSE WITH
                                                        // IT!!!
            ...
            if (hasBricks) {
                ...
                GameRegistry.registerBlock(b3, brickName); // MUST REGISTER BLOCK WITH GAME BEFORE DOING ANYTHING ELSE
                                                           // WITH IT!!!
