...
public class MMDCreativeTab extends CreativeTabs implements IMMDCreativeTab {
    ...
    private static final Comparator<ItemStack> DEFAULT = new Comparator<ItemStack>() {
        ...
        public int compare(ItemStack first, ItemStack second) {
            ...
            return (delta == 0) ? first.getDisplayName()
                .compareToIgnoreCase(second.getDisplayName()) : delta;
        }
    };
    ...
    public static int getSortingValue(@Nonnull final ItemStack itemStack) {
        ...
        if (itemStack.getItem() instanceof ItemBlock && itemStack.getItem() instanceof IMMDObject) {
            classVal = classSortingValues
                .computeIfAbsent(((ItemBlock) itemStack.getItem()).getClass(), (Class<?> clazz) -> 990000);
            materialVal = materialSortingValues
                .computeIfAbsent(((IMMDObject) itemStack.getItem()).getMMDMaterial(), (IMMDMaterial material) -> 9900);
        ...
        return classVal + materialVal + (itemStack.getItemDamage() % 100);
    }
    ...
    public void Initialise() {
        if (iconItem == null && MinIoC.getInstance()
            .resolve(ItemStack.class, "defaultIcon", Mineralogy.MODID) != null)
            this.iconItem = MinIoC.getInstance()
                .resolve(ItemStack.class, "defaultIcon", Mineralogy.MODID);
    ...
    public MMDCreativeTab(@Nonnull final String unlocalizedName, @Nonnull final boolean searchable,
        ...
        if (iconItem != null) this.iconItem = iconItem;
    ...
    public void setTabIconItem() {
        if (this.iconBlock != null) this.setTabIconItem(this.iconBlock);
    }
}
