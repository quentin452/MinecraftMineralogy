...
public final class JsonParser {
    ...
    private static final class PseudoUtf8Reader extends Reader {
        ...
        public int read(char[] cbuf, int off, int len) throws IOException {
            ...
            for (int i = off; i < off + r; i++) cbuf[i] = (char) buf[i];
            return r;
        }
    ...
    private Token advanceToken() throws JsonParserException {
        ...
        switch (c) {
            ...
            case '[': // Inlined function to avoid additional stack
                ...
                if (advanceToken() != Token.ARRAY_END) while (true) {
                    list.add(currentValue());
                    if (advanceToken() == Token.ARRAY_END) break;
                    if (token != Token.COMMA) throw createParseException(
                        null,
            ...
            case '{': // Inlined function to avoid additional stack
                ...
                if (advanceToken() != Token.OBJECT_END) while (true) {
                    if (token != Token.STRING) throw createParseException(null, "Expected STRING, got " + token, true);
                    String key = (String) value;
                    if (advanceToken() != Token.COLON)
                        throw createParseException(null, "Expected COLON, got " + token, true);
                    advanceToken();
                    map.put(key, currentValue());
                    if (advanceToken() == Token.OBJECT_END) break;
                    if (token != Token.COMMA) throw createParseException(
                        null,
        ...
        if (isAsciiLetter(c)) throw createHelpfulException((char) c, null, 0);
    ...
    private void consumeKeyword(char first, char[] expected) throws JsonParserException {
        ...
        for (int i = 0; i < expected.length; i++)
            if (buffer[index++] != expected[i]) throw createHelpfulException(first, expected, i);

        fixupAfterRawBufferRead();

        // The token should end with something other than an ASCII letter
        if (isAsciiLetter(peekChar())) throw createHelpfulException(first, expected, expected.length);
    }
    ...
    private Number consumeTokenNumber(char c) throws JsonParserException {
        ...
        outer: while (true) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (!isDigitCharacter(next)) break outer;
        ...
        try {
            if (isDouble) {
                ...
                if (number.charAt(0) == '0') {
                    if (number.charAt(1) == '.') {
                        if (number.length() == 2) throw createParseException(null, "Malformed number: " + number, true);
            ...
            if (number.length() > 1 && number.charAt(0) == '-' && number.charAt(1) == '0') {
                if (number.length() == 2) return -0.0;
                throw createParseException(null, "Malformed number: " + number, true);
            }

            if (lazyNumbers) return new JsonLazyNumber(number);
                        ...
                        case 'u':
                            ...
                            for (int i = 0; i < 4; i++) {
                                ...
                                if (digit >= '0' && digit <= '9') {
                                    ...
                                    throw createParseException(
                                        null,
                                        "Expected unicode hex escape character: " + (char) digit + " (" + digit + ")",
                                        false);
                                }
                            }
    ...
    private void consumeTokenStringUtf8Char(char c) throws JsonParserException {
        ...
        switch (c & 0xf0) {
            ...
            case 0xb0:
                throw createParseException(
                    null,
                    "Illegal UTF-8 continuation byte: 0x" + Integer.toHexString(c & 0xff),
                    false);
            case 0xc0:
            ...
            case 0xf0:
                ...
                switch ((c & 0xc) >> 2) {
                    case 0:
                    case 1:
                        reusableBuffer.appendCodePoint(
                            (c & 7) << 18 | (buffer[index++] & 0x3f) << 12
                                | (buffer[index++] & 0x3f) << 6
                                | (buffer[index++] & 0x3f));
                        utf8adjust += 3;
                        break;
                    case 2:
                        ...
                        int codepoint = (c & 3) << 24 | (buffer[index++] & 0x3f) << 18
                            | (buffer[index++] & 0x3f) << 12
                            | (buffer[index++] & 0x3f) << 6
                            | (buffer[index++] & 0x3f);
                        throw createParseException(
                            null,
                            "Unable to represent codepoint 0x" + Integer.toHexString(codepoint) + " in a Java string",
                            false);
                    case 3:
                        codepoint = (c & 1) << 30 | (buffer[index++] & 0x3f) << 24
                            | (buffer[index++] & 0x3f) << 18
                            | (buffer[index++] & 0x3f) << 12
                            | (buffer[index++] & 0x3f) << 6
                            | (buffer[index++] & 0x3f);
                        throw createParseException(
        ...
        if (index > bufferLength) throw createParseException(null, "UTF-8 codepoint was truncated", false);
    }
    ...
    private int ensureBuffer(int n) throws JsonParserException {
        ...
        try {
            while (buffer.length > bufferLength) {
                ...
                bufferLength += r;
                if (bufferLength > n) return bufferLength - index;
            }
    ...
    private JsonParserException createParseException(Exception e, String message, boolean tokenPos) {
        if (tokenPos) return new JsonParserException(
            e,
            message + " on line " + tokenLinePos + ", char " + tokenCharPos,
            tokenLinePos,
            tokenCharPos,
            tokenCharOffset);
        else {
