...
class JsonWriterBase<SELF extends JsonWriterBase<SELF>> implements JsonSink<SELF> {
    ...
    public SELF object(String key, Map<?, ?> map) {
        ...
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            ...
            if (!(entry.getKey() instanceof String)) throw new JsonWriterException(
                "Invalid key type for map: " + (entry.getKey() == null ? "null"
                    : entry.getKey()
                        .getClass()));
    ...
    public SELF value(Object o) {
        if (o == null) return nul();
            ...
            .isArray()) {
                ...
                return end();
            } else throw new JsonWriterException("Unable to handle type: " + o.getClass());
    }
    ...
    @Override
    public SELF end() {
        if (states.size() == 0) throw new JsonWriterException("Invalid call to end()");
    ...
    protected void doneInternal() {
        if (states.size() > 0) throw new JsonWriterException("Unclosed JSON objects and/or arrays when closing writer");
        if (first) throw new JsonWriterException("Nothing was written to the JSON writer");
    }
    ...
    private void preValue() {
        if (inObject) throw new JsonWriterException("Invalid call to emit a keyless value while writing an object");

        pre();
    }

    private void preValue(String key) {
        if (!inObject) throw new JsonWriterException("Invalid call to emit a key value while not writing an object");
